...
class fp_GUI(object):
  ...
  '''
  postgui:
  laedt das postgui File
  out: String 		-> Ort des Postgui File
  '''
  def postgui(self):

    self.inifile = linuxcnc.ini(sys.argv[2])
    self.postgui_halfile = self.inifile.find("HAL", "POSTGUI_HALFILE")
    return self.postgui_halfile,sys.argv[2]
  
  
  #def on_cont_speed_value_changed
  '''
  periodic:
...
aa.""" %long(self.x_offset_backe)
  ...
  def __init__(self):#, inifile):
    ...
    self.manual_btn_bt.set_active(True)
    self.setup_speed0_bt = self.builder.get_object("setup_speed0_bt")
    self.setup_speed1_bt = self.builder.get_object("setup_speed1_bt")
    self.setup_speed2_bt = self.builder.get_object("setup_speed2_bt")
    self.setup_speed3_bt = self.builder.get_object("setup_speed3_bt")
    self.setup_speed3_bt.set_active(True)
